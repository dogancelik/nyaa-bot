...
class TopicManager(object):
  ...
  def parse(self, topic):
    firstsplit = map(lambda i: i.strip(), topic.split(' || ', 4))

    if (len(firstsplit) == 5):
      self.title = firstsplit[0]
      self.time = firstsplit[1]
      self.thread = self._varregex.search(firstsplit[2]).groups()[0]
      self.host = self._varregex.search(firstsplit[3]).groups()[0]
      self.other = firstsplit[4] # self._otherregex.search(firstsplit[4]).groups()[0]
